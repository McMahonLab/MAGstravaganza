...
ggplot(data = TBtop, aes(x = Phylum, y = Hits, fill = Layer)) + geom_bar(stat = "identity", position = "dodge" ) + theme_bw() + labs(x = NULL, y = NULL, title = "Trout Bog Read Abundances") + theme(axis.text.x = element_text(angle = 45, hjust= 1, size = 28), axis.text.y = element_text(size = 20), axis.title = element_text(size = 20), plot.title = element_text(size = 32), legend.position = "none") + scale_y_continuous(expand = c(0, 0), limits = c(0, 100000), labels = comma) + scale_fill_manual(values = c("darkred", "dodgerblue"))
dev.off()
...
ggplot(data = MEtop, aes(x = Phylum, y = Hits)) + geom_bar(stat = "identity", fill = c("darkred")) + theme_bw() + labs(x = NULL, y = NULL, title = "Lake Mendota Read Abundances") + theme(axis.text.x = element_text(angle = 45, hjust= 1, size = 24), axis.text.y = element_text(size = 20), axis.title = element_text(size = 20), plot.title = element_text(size = 28), legend.position = "none") + scale_y_continuous(expand = c(0, 0), limits = c(0, 575000), labels = comma)
dev.off()
