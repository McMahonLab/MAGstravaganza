############
# MAGstravaganza_Manuscript_plots_2018-09-18
# Response to reviewers edition
...
Mendota_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/Mendota_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
TE_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/TE_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
TH_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/TH_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
...
genes2keep <- genedata$V1[c(grep("rubisco", genedata$V3), grep("nitrogenase", genedata$V3), grep("Sox", genedata$V3), grep("dissimilatory_sulfite_reductase", genedata$V3), grep("urease", genedata$V3), grep("^sulfite_reductase", genedata$V3), grep("sulfate_adenylyltransferase", genedata$V3), grep("nitrate_reductase", genedata$V3), grep("nitrite_reductase", genedata$V3), grep("nitric_oxide_reductase", genedata$V3), grep("nitrous_oxide_reductase", genedata$V3), grep("sulfide_quinone_reductase", genedata$V3))]
...
table2keep$annotations <- gsub("rubisco", "RuBisCO", table2keep$annotations)
...
save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/fig1_revised.pdf", fig1, base_height = 6, base_aspect_ratio = 1.5)

# Use Wilcoxon Rank Sum Test to look for significant differences in distributions between lakes
genes <- c("RuBisCO", "urease", "nitrogenase", "nitrate reductase", "nitrite reductase", "nitric oxide reductase", "nitrous oxide reductase", "SOX", "sulfate adenylyltransferase", "sulfide quinone reductase", "sulfite reductase")
...
#Load data
table <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Pathway_analysis/consolidated_pathway_data.csv", header = T)
metadata <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Supplemental/MAG_information.csv")
...
save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/ME.heatmap.pdf", ME.heatmap, base_height = 8, base_aspect_ratio = 0.75)
...
save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/TE.heatmap.pdf", TE.heatmap, base_height = 8, base_aspect_ratio = 0.7)
...
save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/TH.heatmap.pdf", TH.heatmap, base_height = 8, base_aspect_ratio = .75)
...
# Load data
mag_data <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Supplemental/MAG_information.csv", header = T)
...
Mendota_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/Mendota_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
Mendota_dates <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/ME.sampledata.filtered.tsv", sep = "\t", header = T)
Mendota_dates$sample <- as.character(Mendota_dates$sample)
metadata <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Supplemental/MAG_information.csv", header = T)
...
# Note: order of figures S2 and S3 swapped late in the revision process
# They are presented out of order here
### Fig S3. 16S classifications
...
mendota_tags <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Supplemental/Mendota_OTUtable.csv", header = T)
trout_tags <- read.csv("/Users/Alex/Desktop/MAGstravaganza/Supplemental/TroutBog_OTUtable.csv", header = T)
...
tag_data$phylum_names <- factor(tag_data$phylum_names, levels = c(phyla))

figS2 <- ggplot(data = tag_data[which(tag_data$phylum_names != "unclassified.Proteobacteria"), ], aes(x = phylum_names, y = phylum_percents, fill = phylum_lake)) + geom_col(position = "dodge") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(y = "Mean Percent of Reads", x = NULL) + scale_fill_manual(values = c("#b2df8a", "#a6cee3", "#8856a7"))  + geom_vline(xintercept = c(1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 14.5), col='grey', lwd=1, linetype="dotted") + scale_y_continuous(expand = c(0, 0), limits = c(0, 40)) + guides(fill=guide_legend(title="Lake")) + theme(plot.margin=unit(c(1,0,0,1), "cm"))

save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/figS2_revised.pdf", figS2, base_height = 4, base_aspect_ratio = 1.6)


### Figure S2. MAG classifications by read coverage
...
TE_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/TE_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
TE_dates <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/tb_epi.sample_data.filtered.txt", header = T, sep = "\t")
TE_dates$sample <- as.character(TE_dates$sample)

TH_raw_input <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/TH_results.txt", colClasses = c("character", "character", "numeric", "numeric"))
TH_dates <- read.table("/Users/Alex/Desktop/MAGstravaganza/time_series_mapping/tb_hyp.sample_data.filtered.txt", header = T, sep = "\t")
...
figS3 <- ggplot(data = agg_phyla, aes(x = phylum, y = RPKM, fill = Lake)) + geom_col(position = "dodge") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(y = "Mean Normalized Reads", x = NULL) + scale_fill_manual(values = c("#b2df8a", "#a6cee3", "#8856a7")) + geom_vline(xintercept = c(1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 14.5), col='grey', lwd=1, linetype="dotted") + scale_y_continuous(expand = c(0, 0)) + theme(legend.text=element_text(size=12))

save_plot("/Users/Alex/Desktop/MAGstravaganza/Manuscript_plots/figS3.pdf", figS3, base_height = 4, base_aspect_ratio = 1.6)
